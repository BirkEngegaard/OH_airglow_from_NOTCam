function [counts74, spec_relerr74, counts85, spec_relerr85, specificDark, exposureTime, obsTime_mean, airmass, SAT, rampMode]...
    = preparePic_J(path,file,filter,dark,hot,flat,bad, figpath, S, data, showFig, useMasterDark, nightMaster)

obsTime_mean = 0;
if useMasterDark
    specificDark = 0; % Using master dark
elseif nightMaster
    specificDark = 2; % Using a dark from the same night
else
    specificDark = 1; % Found a specific dark
end
...
for p = pixRough'
    ...
    ff=fit(y,short,gauss,'StartPoint',[70000, p, 1.5, me],'Lower',[5000,p-10,1,me*0.1],'Upper',[100000,p+10,4,4*me]);
%     figure;
%     plot(ff,y,short);
    coeffs = coeffvalues(ff);
    pix(i)=coeffs(2);
    i=i+1;
end
lam = flipud([1.30214;1.29057;1.24228;1.23512;1.22867;1.22291;1.21226]);
%lam = [1.30214;1.29057;1.24228;1.23512;1.22867;1.22291;1.21226];

poly3 = fittype('a*x^3+b*x^2+c*x+d','coeff',{'a','b','c','d'});
ff=fit(pix,lam,poly3,'StartPoint',[-1.6e-11,8.5e-08,3.4e-4,1.9]);
% figure;
% plot(ff,pix,lam);
...
%lambda = flipud(lambda); % Flip so lambda increases towards the right
spec = flipud(spec);
...
if showFig
    ...
    ylabel('Intensity [photons/s/m$^2$/sr/$\mu$ m]', 'Interpreter', 'latex')
end
...
elseif filter == 'K'
    ...
    q_97_peakrange = 425:483;
    q97spec = spec; 
    fitq97spec = fit(transpose([pix(Q_97_range)]),q97spec(Q_97_range),'poly1');
    q97spec = q97spec - fitq97spec(pix);
    q97spec = q97spec + abs(min(q97spec(Q_97_range)));
    thresh = 0.05*mean(spec(Q_97_range));
    [~, locs, ~, p] = findpeaks(spec(q_97_peakrange), 'MinPeakWidth', 1,'MinPeakProminence',thresh);%,'MinPeakWidth', 1); %,'SortStr', 'descend');
    [~, loci] = max(p);
    q97i = locs(loci);
    %[~, q31i] = max(spec(q_31_peakrange));
    q97i = q97i + q_97_peakrange(1) - 1;
    if strcmp(file, 'NCyg030363') || strcmp(file, 'NCwf220699') || strcmp(file, 'NCwf220695') || strcmp(file,  'NCyh020745') || strcmp(file,  'NCyh020744')
        q97i = 476;
    elseif strcmp(file, 'NCre210686') || strcmp(file,  'NCsa100269') || strcmp(file, 'NCsa100052') || strcmp(file, 'NCsa100270')...
            || strcmp(file, 'NCrg160276')
        q97i = 467;
    %elseif strcmp(file, 'NCrf090674') || strcmp(file,'NCrf090675') || strcmp(file,'NCrf090676') || strcmp(file(1:6),'NCrf09')
    elseif strcmp(file(1:6),'NCrf09') || strcmp(file, 'NCsa100169')
        q97i = 466;
    elseif strcmp(file(1:6), 'NCve08')
        q97i = 475;   
    end
    %Q_31_spec = (q31i-5):(q31i+25);
    %Q_97_spec = (q97i-22):(q97i+5);
    Q_97_spec = (q97i-34):(q97i+5);
    q97range = [q97i - 30, Q_97_spec(1) - 1 , Q_97_spec(end) + 1, q97i + 22];
    q97fitrange = [q97i - 45, q97i - 34, Q_97_spec(end) + 1, q97i + 19]; % Needs to be q97i + 19 or less
    if exposureTime <= 40 && exposureTime > 35
        Q_97_spec = (q97i-20):(q97i+4); 
        q97range = [q97i - 30, Q_97_spec(1) - 1 , Q_97_spec(end) + 1, q97i + 22];
        q97fitrange = [q97i - 30, q97i - 19, Q_97_spec(end) + 1, q97i + 10];
    %% 7.0 10
    elseif str2double(expMode{2}) == 7.0 && str2double(expMode{3}) == 10 % Total time = time of ramp * num of ramps
        Q_97_spec = (q97i-21):(q97i+4); 
        q97range = [q97i - 30, Q_97_spec(1) - 1 , Q_97_spec(end) + 1, q97i + 22];
        q97fitrange = [q97i - 28, q97i - 21, Q_97_spec(end) + 1, q97i + 9];
    %% 10.0 3
    elseif str2double(expMode{2}) == 10.0 && str2double(expMode{3}) == 3 % Total time = time of ramp * num of ramps
        Q_97_spec = (q97i-22):(q97i+4); 
        q97range = [q97i - 30, Q_97_spec(1) - 1 , Q_97_spec(end) + 1, q97i + 22];
        %q97fitrange = [q97i - 28, q97i - 12, Q_97_spec(end) + 1, q97i + 14];
        q97fitrange = [q97i - 28, q97i - 21, Q_97_spec(end) + 1, q97i + 14];
    %% 10.0 6
    elseif str2double(expMode{2}) == 10.0 && str2double(expMode{3}) == 6 % Total time = time of ramp * num of ramps
        Q_97_spec = (q97i-35):(q97i+4); 
        q97range = [q97i - 20, Q_97_spec(1) - 1 , Q_97_spec(end) + 1, q97i + 22];
        q97fitrange = [q97i - 30, q97i - 20, Q_97_spec(end) + 1, q97i + 11];
    %% 10.0 7
    elseif str2double(expMode{2}) == 10.0 && str2double(expMode{3}) == 7 % Total time = time of ramp * num of ramps
        Q_97_spec = (q97i-35):(q97i+4); 
        q97range = [q97i - 20, Q_97_spec(1) - 1 , Q_97_spec(end) + 1, q97i + 22];
        q97fitrange = [q97i - 31, q97i - 21, Q_97_spec(end) + 1, q97i + 11];
    %% 3.6 3
    elseif str2double(expMode{2}) == 3.6 && str2double(expMode{3}) == 3 % Total time = time of ramp * num of ramps
        Q_97_spec = (q97i-27):(q97i+4); 
        q97range = [q97i - 30, Q_97_spec(1) - 1 , Q_97_spec(end) + 1, q97i + 22];
        q97fitrange = [q97i - 28, q97i - 20, Q_97_spec(end), q97i + 11];
    %% 4.0 3
    elseif str2double(expMode{2}) == 4.0 && str2double(expMode{3}) == 3 % Total time = time of ramp * num of ramps
        Q_97_spec = (q97i-18):(q97i+4); 
        q97range = [q97i - 30, Q_97_spec(1) - 1 , Q_97_spec(end) + 1, q97i + 22];
        q97fitrange = [q97i - 20, q97i - 18, Q_97_spec(end), q97i + 10];
    %% 5.0 4
    elseif str2double(expMode{2}) == 5.0 && str2double(expMode{3}) == 4 % Total time = time of ramp * num of ramps
        Q_97_spec = (q97i-17):(q97i+4); 
        q97range = [q97i - 21, Q_97_spec(1) - 1 , Q_97_spec(end) + 1, q97i + 22];
        q97fitrange = [q97i - 19, q97i - 13, Q_97_spec(end), q97i + 10];
    %% 5.0 6
    elseif str2double(expMode{2}) == 5.0 && str2double(expMode{3}) == 6 % Total time = time of ramp * num of ramps
        Q_97_spec = (q97i-20):(q97i+4); 
        q97range = [q97i - 21, Q_97_spec(1) - 1 , Q_97_spec(end) + 1, q97i + 22];
        q97fitrange = [q97i - 23, q97i - 13, Q_97_spec(end), q97i + 15];
    %% 6.0 3
    elseif str2double(expMode{2}) == 6.0 && str2double(expMode{3}) == 3 % Total time = time of ramp * num of ramps
        Q_97_spec = (q97i-19):(q97i+4); 
        q97range = [q97i - 20, Q_97_spec(1) - 1 , Q_97_spec(end) + 1, q97i + 22];
        q97fitrange = [q97i - 21, q97i - 13, Q_97_spec(end), q97i + 10];
    %% 4.0 2
    elseif str2double(expMode{2}) == 4.0 && str2double(expMode{3}) == 2 % Total time = time of ramp * num of ramps
        Q_97_spec = (q97i-20):(q97i+4); 
        q97range = [q97i - 20, Q_97_spec(1) - 1 , Q_97_spec(end) + 1, q97i + 22];
        q97fitrange = [q97i - 20, q97i - 13, Q_97_spec(end), q97i + 10];
    %% 17.0 5
    elseif str2double(expMode{2}) == 17.0 && str2double(expMode{3}) == 5 % Total time = time of ramp * num of ramps
        Q_97_spec = (q97i-35):(q97i+4); 
        q97range = [q97i - 20, Q_97_spec(1) - 1 , Q_97_spec(end) + 1, q97i + 22];
        q97fitrange = [q97i - 40, q97i - 20, Q_97_spec(end), q97i + 15];
    %% 60 to 40 s
    elseif exposureTime > 40 && exposureTime <= 60
        Q_97_spec = (q97i-20):(q97i+5); 
        q97range = [q97i - 30, Q_97_spec(1) - 1 , Q_97_spec(end) + 1, q97i + 22];
        q97fitrange = [q97i - 30, q97i - 20, Q_97_spec(end) + 1, q97i + 11]; 
    %% 35 to 20
    elseif exposureTime <= 35 && exposureTime > 20
        Q_97_spec = (q97i-20):(q97i+4); 
        q97range = [q97i - 30, Q_97_spec(1) - 1, Q_97_spec(end) + 1, q97i + 22];
        q97fitrange = [q97i - 30, Q_97_spec(1), Q_97_spec(end), q97i + 9];
    %% 20 to 10
    elseif exposureTime <= 20 && exposureTime > 10
        Q_97_spec = (q97i-20):(q97i+5); 
        q97range = [q97i - 30, Q_97_spec(1) - 1 , Q_97_spec(end) + 1, q97i + 22];
        q97fitrange = [q97i - 20, q97i - 14, Q_97_spec(end), q97i + 10];
    %% 10 to 3.6
    elseif exposureTime <= 10 && exposureTime > 3.6
        Q_97_spec = (q97i-20):(q97i+4); 
        q97range = [q97i - 30, Q_97_spec(1) - 1 , Q_97_spec(end) + 1, q97i + 22];
        q97fitrange = [q97i - 25, q97i - 14, Q_97_spec(end), q97i + 12];
    elseif exposureTime <= 3.6
        Q_97_spec = (q97i-19):(q97i+2); 
        q97range = [q97i - 30, Q_97_spec(1) - 1 , Q_97_spec(end) + 1, q97i + 22];
        q97fitrange = [q97i - 29, q97i - 20, Q_97_spec(end) + 1, q97i + 8];
    end
    
    q97range = q97fitrange;
    

%     warning('off','all')
%     noise_per_pix_97_3 = fit(transpose([q97range(1):q97range(2) q97range(3):q97range(4)]),...
%         transpose([specT(q97range(1):q97range(2)) specT(q97range(3):q97range(4))]), 'poly3');
%     
%     noise_per_pix_97_2 = fit(transpose([q97range(1):q97range(2) q97range(3):q97range(4)]),...
%         transpose([specT(q97range(1):q97range(2)) specT(q97range(3):q97range(4))]), 'poly2');
%     warning('on','all')

    warning('off','all')
    noise_per_pix_97_3 = fit(transpose([q97fitrange(1):q97fitrange(2) q97fitrange(3):q97fitrange(4)]),...
        transpose([specT(q97fitrange(1):q97fitrange(2)) specT(q97fitrange(3):q97fitrange(4))]), 'poly3');
    
    noise_per_pix_97_2 = fit(transpose([q97fitrange(1):q97fitrange(2) q97fitrange(3):q97fitrange(4)]),...
        transpose([specT(q97fitrange(1):q97fitrange(2)) specT(q97fitrange(3):q97fitrange(4))]), 'poly2');
    warning('on','all')

%     nc_spec31 = spec - noise_per_pix_31(pix); % This is only valid in the 31 Q branch region!
%     nc_spec31h = transpose(nc_spec31);
    
    nc_spec97_2 = spec - noise_per_pix_97_2(pix); % This is only valid in the 31 Q branch region!
    nc_spec97h_2 = transpose(nc_spec97_2);
    
    nc_spec97_3 = spec - noise_per_pix_97_3(pix); % This is only valid in the 31 Q branch region!
    nc_spec97h_3 = transpose(nc_spec97_3);
    
    dlambda = abs(((lambda(Q_97_spec(end))-lambda(Q_97_spec(1)))/(length(Q_97_spec)-1)));
    counts97 = sum(nc_spec97_3(Q_97_spec))*dlambda;
    
    counts97_corr23 = mean([sum(nc_spec97_2(Q_97_spec))*dlambda, sum(nc_spec97_3(Q_97_spec))*dlambda]);
    
    stderror97 = std(transpose([nc_spec97h_3(q97range(1):q97range(2)) nc_spec97h_3(q97range(3):q97range(4))]))...
        /sqrt(length(transpose([Q_97_spec])));
    
    counts97_1 = counts97;
    counts97 = counts97_corr23; % Use mean of poly2 and poly3 fit to get the counts.
    
    spec_stderr97 = stderror97*length(Q_97_spec)*dlambda + (std([counts97,counts97_corr23])/sqrt(2));
    spec_relerr97 = spec_stderr97/counts97;
    
    %% Using the difference between the two estimates as error estimate
    ints = [counts97_1, counts97];
    error_estimate_by_diff = std(ints)/sqrt(2); % Standard error of the two intensities
    rel_error_estimate_by_diff = error_estimate_by_diff/counts97;
    
    if rel_error_estimate_by_diff > spec_relerr97
        spec_relerr97 = rel_error_estimate_by_diff;
    end
    
%     fprintf(' Total photons/sec/m2/sr from 31 = %d \n Relative error = %0.2f %% \n',counts31, spec_relerr31*100);
%  
    if showFig
        sprintf('Specific dark: %d, %s, 97: $I_{3}$ = %g, $I_{2+3}$ = %g',specificDark, S.PrimaryData.Keywords{eindex, 2},counts97_1,counts97)
        figure;
        plot(Q_97_range, specT(Q_97_range), Q_97_range, noise_per_pix_97_2(Q_97_range),Q_97_range, noise_per_pix_97_3(Q_97_range))
        title(sprintf('%s 97 Q-branch, rel.err = %0.2f %%',file,spec_relerr97*100))
        xlabel('Pix')
        ylabel('Intensity [a.u.]')  
        y1=get(gca,'ylim');
        hold on % Vertical lines to visualize integration area
        plot([(Q_97_spec(1)) (Q_97_spec(1))],y1, '--m', 'linewidth', 1)
        plot([(q97range(3)-1) (q97range(3)-1)],y1, '--m', 'linewidth', 1)
        hold off
        
        figure;
        plot(lambdaT(Q_97_range), specT(Q_97_range), lambda(Q_97_range), noise_per_pix_97_2(Q_97_range),lambdaT(Q_97_range), noise_per_pix_97_3(Q_97_range))
        title(sprintf('%s 97 Q-branch, rel.err = %0.2f %%',file,spec_relerr97*100))
        xlabel('$\lambda$ [$\mu$m ]')
        ylabel('Intensity [photons/s/m$^2$/sr/$\mu$m]')  
        y1=get(gca,'ylim');
        hold on % Vertical lines to visualize integration area
        plot([lambdaT((Q_97_spec(1))) lambdaT((Q_97_spec(1)))],y1, '--m', 'linewidth', 1)
        plot([lambdaT(q97range(3)-1) lambdaT(q97range(3)-1)],y1, '--m', 'linewidth', 1)
        hold off
    end
...
elseif filter == 'J'
    ...
    lambda = flipud(lambda); % Align lambda with pixel and spec
    counts74 = 221193;
    spec_relerr74 = 221193;
    counts85 = 221193;
    spec_relerr85 = 221193;
else
%% Other filters
    %% Save processed data
